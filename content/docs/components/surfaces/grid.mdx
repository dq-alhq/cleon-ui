...
## Installation
<Install cli='grid' deps={['react-aria-components']} components={['grid']} />
...
<Note>This grid ain't got no default variants, so if you don't set an initial value, it's a big fat nothing. When you're setting up a responsive prop like `sm` or `md`, make sure to slap an initial value for the breakpoint.</Note>
